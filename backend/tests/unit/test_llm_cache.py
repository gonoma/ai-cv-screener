...
def test_the_same_prompt_is_answered_once(monkeypatch, provider):
    seen = count_calls(monkeypatch, provider)

    first = provider.generate_json_object("who knows Python?", SCHEMA)
    second = provider.generate_json_object("who knows Python?", SCHEMA)
...
def test_a_changed_prompt_is_not_served_from_cache(monkeypatch, provider):
    ...
    provider.generate_json_object("who knows Python?", SCHEMA)
    provider.generate_json_object("who knows Python? Answer concisely.", SCHEMA)
...
def test_an_abandoned_stream_is_not_cached(monkeypatch, provider):
    ...
    monkeypatch.setattr(
        ...
        lambda self, s, u: iter(["one ", "two ", "three"]),
    )

    stream = provider.stream_text_tokens("system", "user")
    assert next(stream) == "one "
    stream.close()

    replayed = "".join(provider.stream_text_tokens("system", "user"))
